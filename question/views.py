...
class Questions(AsyncAPIView):
    ...
    async def get(self, request):
        ...
        response_data = await sync_to_async(lambda: [item["question_text"] for item in questions_serializer.data])()
        response = {
    ...
    async def post(self, request):
        ...
        return Response(
            {"poll_id": await sync_to_async(encrypt_resource_id)(poll_id)}, status=status.HTTP_201_CREATED
        )
